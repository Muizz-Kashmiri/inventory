...
function App() {
  ...
  const fetchConnectedRegion = async () => {
    try {
      const response = await axios.get('https://{{EDUCATIVE_LIVE_VM_ALT_SERVER_URL}}/connected_region');
  ...
  const handleGetSong = async () => {
    try {
      const response = await axios.get(`https://{{EDUCATIVE_LIVE_VM_ALT_SERVER_URL}}/songs/${songName}`);
  ...
  const handleAddSong = async () => {
    try {
      await axios.post('https://{{EDUCATIVE_LIVE_VM_ALT_SERVER_URL}}/songs', { songName, artistName, yearOfRelease });
